...
export default function MedicacionesDiarias() {
    ...
    try {
      ...
      arregloMedicacion.forEach((element) => {
        if (element.Medicamento == medicacionParam) {
          suma +=
            parseInt(element["6:00"]) +
            parseInt(element.Desayuno) +
            parseInt(element.Almuerzo) +
            parseInt(element.Merienda) +
            parseInt(element.Cena) +
            parseInt(element["22:30"]);
        }
      });
  ...
  return (
    <>
      <div className="border w-full rounded-lg p-3 shadow-md overflow-x-auto">

...
const TablaMedicamentos = forwardRef(
  ({
    ...
    const handleEditRow = (index) => {
      setMedicamentos((prev) => {
        const nuevosMedicamentos = [...prev];
        nuevosMedicamentos[index].editable =
          !nuevosMedicamentos[index].editable;
        return nuevosMedicamentos;
      });
    };
    ...
    async function guardarMedicamentos() {
      ...
      const response = await guardarMedicamentosApi(
        arregloMedicacion,
        parseInt(paciente.dni)
      );
                ...
                medicamento.medicamento && (
                  <tr key={index}>
                    <td className="border border-gray-300 p-0 h-full align-middle">
                      <select
                        className={`w-full h-full py-3 box-border text-center text-gray-500 font-bold ${
                    ...
                    {horasDelDia.map((hora, idx) => (
                      <td
                        key={idx}
                        className="border border-gray-300 p-0 h-full align-middle"
                      >
                        <input
                    ...
                    <td
                      className={`w-full flex border border-gray-300 p-0 h-full align-middle ${
                        !medicamento.editable ? "bg-gray-200" : "bg-white"
                      }`}
                    >
                      <BotonEditar
                        editable={medicamento.editable}
                        onGuardar={() => handleEditRow(index)}
                        onModificar={() => handleEditRow(index)}
                      />

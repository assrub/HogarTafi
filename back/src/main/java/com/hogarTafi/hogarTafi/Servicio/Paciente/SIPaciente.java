...
@Service
public class SIPaciente implements SPacientes {
    ...
    public boolean modificarPaciente(Integer dni, String nombre, String apellido, String obraSocial, String observaciones, byte[] fotoFrenteCarnetBytes, byte[] fotoAtrasCarnetBytes, byte[] fotoFrenteDniBytes, byte[] fotoAtrasDniBytes) 
    {
        // Obtener el paciente existente
        EPaciente paciente = repositorioPacientes.findByDni(dni).orElseThrow(() -> new NoSuchElementException("Paciente no encontrado."));
        ...
        public boolean desactivarPaciente(OcultarPacienteConsulta consulta) {
            EPaciente existePaciente = buscarPaciente(consulta.getDni());
